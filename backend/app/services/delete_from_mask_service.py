...
def remove_with_mask(payload: RemoveWithMaskRequest) -> str:
    ...
    input_image = _load_image(input_image_url).convert("RGB")
    #L is grayscale modality in PIL(luminosity)
    mask_image = _load_image(mask_image_url).convert("L")

    #If sizes are different, resize of the mask
    if input_image.size != mask_image.size:
        mask_image = mask_image.resize(input_image.size)

   # 255 where mask from segmentation is > 0 
    ...
    output_bytes = _image_to_png_bytes(result)
    output_path = save_project_bytes(payload.project_id, output_bytes, "delete-from-mask.png")
    return output_path


def _load_image(source: str) -> Image.Image:
    if source.startswith("data:"):
        return _load_image_from_data_uri(source)

    if source.startswith("/uploads/"):
        return _load_image_from_uploads_path(source)

    parsed = urlparse(source)
    if parsed.scheme in ("http", "https"):
        if parsed.path.startswith("/uploads/"):
            return _load_image_from_uploads_path(parsed.path)
        return _load_image_from_remote_url(source)
...
def _load_image_from_uploads_path(public_path: str) -> Image.Image:
...
def _load_image_from_remote_url(url: str) -> Image.Image:
    try:
